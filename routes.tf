...
resource "aws_route_table" "inbound_external_route_table" {
  ...
  tags {
    Name = "${var.project_name}-Inbound-External-Route-Table"
  }
}
...
resource "aws_route_table_association" "inbound_external_table_association" {
  ...
  route_table_id = "${aws_route_table.inbound_external_route_table.id}"
}
...
resource "aws_route_table_association" "private_route_table_association" {
  count          = "${length(data.aws_availability_zones.azs.names)}"
  subnet_id      = "${element(aws_subnet.private_external_subnet.*.id, count.index)}"
...
resource "aws_route_table" "test_route_table" {
  ...
  route {
    cidr_block          = "${var.Current_Public_IP}"
